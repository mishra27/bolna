...
class CartesiaSynthesizer(BaseSynthesizer):
    ...
    async def sender(self, text, ctx, end_of_llm_stream=False):
    ...
    async def receiver(self, ctx=None):
        async for output in ctx.receive():
            buffer = output["audio"]
            chunk = base64.b64decode(buffer)
            yield chunk
        ...
        # while True:
        #     try:
        #         if self.websocket_holder["websocket"] is None or not self.websocket_holder["websocket"].cr_running:
    ...
    async def establish_connection(self):
        try:
            client = AsyncCartesia(api_key="6a1b383a-0ba2-4457-a0bc-45ad8ef682cf")
    ...
    async def push(self, message):
        ...
        if self.stream:
            ...
            meta_info["text"] = text
            self.context_id = self.websocket_holder["websocket"].context()
            self.sender_task = asyncio.create_task(self.sender(text, self.context_id, end_of_llm_stream))
            self.text_queue.append(meta_info)
            self.listen_task = asyncio.create_task(self.receiver(self.context_id))
            await asyncio.gather(self.sender_task, self.listen_task)
